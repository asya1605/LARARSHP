...
<section class="min-h-[90vh] bg-[#f5f7ff] flex justify-center items-center">
  <div class="bg-white shadow-md rounded-xl p-8 w-full max-w-md border border-gray-200">
    ...
    <form method="POST" action="{{ route('admin.kategori_klinis.update', $kategori->idkategori_klinis) }}">
      @csrf

      <div class="mb-4">
        <label class="block text-sm font-semibold text-gray-700 mb-1">Nama Kategori Klinis</label>
        <input type="text" name="nama_kategori_klinis" value="{{ old('nama_kategori_klinis', $kategori->nama_kategori_klinis) }}" class="w-full border rounded-lg px-3 py-2" required>
      </div>

      <div class="flex justify-between mt-4">
        <a href="{{ route('admin.kategori_klinis.index') }}" class="text-gray-600 hover:text-gray-800 text-sm">← Batal</a>
