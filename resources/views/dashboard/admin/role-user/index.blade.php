...
<section class="min-h-[90vh] bg-[#f5f7ff]">
  ...
  <div class="bg-[#002080] text-white py-3 shadow-md">
    <div class="max-w-7xl mx-auto flex justify-center items-center gap-10 font-semibold text-[15px]">
      ...
      <form action="{{ route('admin.role-user.store') }}" method="POST" class="space-y-5">
        ...
        <div class="grid sm:grid-cols-2 md:grid-cols-3 gap-5">
          <div>
            ...
            <select name="iduser" required class="w-full border rounded-lg px-3 py-2">
          ...
          <div>
            ...
            <select name="idrole" required class="w-full border rounded-lg px-3 py-2">
          ...
          <div class="flex items-center mt-6 space-x-2">
            ...
            <span class="text-sm">Jadikan aktif</span>
          </div>
        </div>
        <div class="text-right">
          <button class="bg-[#002080] hover:bg-[#00185e] text-white px-5 py-2 rounded-lg text-sm">Simpan</button>
        </div>
      </form>
    </div>
              ...
              <td class="py-3 px-4">
                @forelse($u->roles as $role)
                  <div class="flex items-center justify-between bg-gray-50 rounded-lg px-4 py-2 mb-2 border border-gray-200">
                    <div>
                      <span class="font-semibold">{{ $role->nama_role }}</span>
                ...
                @empty
                  <span class="text-gray-400 text-sm">Belum punya role</span>
                @endforelse
              </td>
            </tr>
          @empty
            <tr><td colspan="4" class="text-center py-5 text-gray-500">Belum ada data user.</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
